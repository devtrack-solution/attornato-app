...
@Component({
  selector: 'app-procedural-status-new',
  standalone: true,
  imports: [
    CommonModule,
    SharedModule,
  ...
  templateUrl: './procedural-status-new.component.html',
  styleUrl: './procedural-status-new.component.scss'
})
export class ProceduralStatusNewComponent implements OnInit {
  ...
  async onSubmit(name: any): Promise<void> {
    ...
    try {
      await this.proceduralStatusService.saveProceduralStatus({ name: name });
      this.messageService.add({ severity: 'success', summary: 'Sucesso', detail: 'Status Processual cadastrado com sucesso!' });
      setTimeout(() => {
        this.display = false
        location.reload();
      }, 1000);
