...
@Component({
  ...
  styleUrl: './details-new.component.scss'
})
export class DetailsNewComponent implements OnInit {
  ...
  async onSubmit(name: any): Promise<void> {
    ...
    try {
      await this.detailsService.saveDetail({ name: name });
      this.messageService.add({ severity: 'success', summary: 'Sucesso', detail: 'Detalhe cadastrado com sucesso!' });
      setTimeout(() => {
        this.display = false
        location.reload();
      }, 1000);
