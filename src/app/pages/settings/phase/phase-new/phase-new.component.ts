...
@Component({
  ...
  styleUrl: './phase-new.component.scss'
})
export class PhaseNewComponent implements OnInit {
   ...
   form = new FormGroup({})
    ...
    async onSubmit(name: any): Promise<void> {
      ...
      try {
        await this.phaseService.savePhase({ name: name });
        this.messageService.add({ severity: 'success', summary: 'Sucesso', detail: 'Fase cadastrado com sucesso!' });
        setTimeout(() => {
          this.display = false
          location.reload();
        }, 1000);
