...
@Component({
  ...
  styleUrl: './phase-edit.component.scss'
})
export class PhaseEditComponent implements OnInit{
  ...
  form: FormGroup;
    ...
    async openLg(phase: any): Promise<void> {
      this.model = phase
      this.form.patchValue({ name: phase.name }); // Preenche o formulário com os dados do processo
      this.display = true
    }
    ...
    async onSubmit(name: any): Promise<void> {
      ...
      try {
        await this.phaseService.ediPhase(this.model.id, { name: name });
        this.messageService.add({ severity: 'success', summary: 'Sucesso', detail: 'Fase atualizado com sucesso!' });
        setTimeout(() => {
          this.display = false
          location.reload();
        }, 1000);
      } catch (error) {
        console.log('Erro ao salvar Fase', error);
        this.messageService.add({ severity: 'error', summary: 'Erro', detail: 'Erro ao salvar um Fase!' });
      }
    }
