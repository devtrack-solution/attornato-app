...
@Component({
  ...
  templateUrl: './prognosis-edit.component.html',
  styleUrl: './prognosis-edit.component.scss'
})
export class PrognosisEditComponent implements OnInit {
  ...
  async onSubmit(name: any): Promise<void> {
    ...
    try {
      ...
      this.messageService.add({ severity: 'success', summary: 'Sucesso', detail: 'Prognóstico atualizado com sucesso!' });
      setTimeout(() => {
        this.display = false
        location.reload();
      }, 1000);
    } catch (error) {
      console.log('Erro ao salvar Prognóstico', error);
      this.messageService.add({ severity: 'error', summary: 'Erro', detail: 'Erro ao salvar um Prognóstico!' });
    }
  }
