...
@Component({
  selector: 'app-practice-area-edit',
  standalone: true,
  imports: [
    CommonModule,
    SharedModule,
  ...
  templateUrl: './practice-area-edit.component.html',
  styleUrl: './practice-area-edit.component.scss'
})
export class PracticeAreaEditComponent implements OnInit {
  ...
  async openLg(practiceArea: any): Promise<void> {
    this.model = practiceArea
    this.form.patchValue({ name: practiceArea.name }); // Preenche o formulário com os dados do processo
    this.display = true
  }
  ...
  async onSubmit(name: any): Promise<void> {
    ...
    try {
      await this.practiceAreaService.ediPracticeArea(this.model.id, { name: name });
      this.messageService.add({ severity: 'success', summary: 'Sucesso', detail: 'Área de Atuação atualizado com sucesso!' });
      setTimeout(() => {
        this.display = false
        location.reload();
      }, 1000);
    } catch (error) {
      console.log('Erro ao salvar Área de Atuação', error);
      this.messageService.add({ severity: 'error', summary: 'Erro', detail: 'Erro ao salvar um Área de Atuação!' });
    }
  }
