...
@Component({
  selector: 'app-practice-area-new',
  standalone: true,
  imports: [
    CommonModule,
    SharedModule,
  ...
  templateUrl: './practice-area-new.component.html',
  styleUrl: './practice-area-new.component.scss'
})
export class PracticeAreaNewComponent implements OnInit {
  ...
  async onSubmit(name: any): Promise<void> {
    ...
    try {
      await this.practiceAreaService.savePracticeArea({ name: name });
      this.messageService.add({ severity: 'success', summary: 'Sucesso', detail: 'Área de Atuação cadastrado com sucesso!' });
      setTimeout(() => {
        this.display = false
        location.reload();
      }, 1000);
