...
@Component({
  ...
  styleUrl: './responsible-edit.component.scss'
})
export class ResponsibleEditComponent implements OnInit {
  ...
  async openLg(responsible: any): Promise<void> {
    this.model = responsible
    this.form.patchValue({ name: responsible.name }); // Preenche o formulário com os dados do processo
    this.display = true
  }
  ...
  async onSubmit(name: any): Promise<void> {
    ...
    try {
      await this.responsibleService.ediResponsible(this.model.id, { name: name });
      this.messageService.add({ severity: 'success', summary: 'Sucesso', detail: 'Responsável atualizado com sucesso!' });
      setTimeout(() => {
        this.display = false
        location.reload();
      }, 1000);
