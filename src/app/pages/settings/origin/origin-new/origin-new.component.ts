...
@Component({
  ...
  styleUrl: './origin-new.component.scss'
})
export class OriginNewComponent implements OnInit {
  ...
  async onSubmit(name: any): Promise<void> {
    ...
    try {
      await this.originService.saveOrigin({ name: name });
      this.messageService.add({ severity: 'success', summary: 'Sucesso', detail: 'Origem cadastrado com sucesso!' });
      setTimeout(() => {
        this.display = false
        location.reload();
      }, 1000);
