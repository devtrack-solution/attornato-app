...
@Component({
  selector: 'app-object-action-edit',
  standalone: true,
  imports: [
    CommonModule,
    SharedModule,
  ...
  templateUrl: './object-action-edit.component.html',
  styleUrl: './object-action-edit.component.scss'
})
export class ObjectActionEditComponent implements OnInit {
  ...
  objectActionService = inject(ObjectActionService)
  ...
  async onSubmit(name: any): Promise<void> {
    ...
    try {
      await this.objectActionService.ediObjectAction(this.model.id, { name: name });
      this.messageService.add({ severity: 'success', summary: 'Sucesso', detail: 'Objeto de Ação atualizado com sucesso!' });
      setTimeout(() => {
        this.display = false
        location.reload();
      }, 1000);
    } catch (error) {
      console.log('Erro ao salvar Objeto de Ação', error);
      this.messageService.add({ severity: 'error', summary: 'Erro', detail: 'Erro ao salvar um Objeto de Ação!' });
    }
  }
