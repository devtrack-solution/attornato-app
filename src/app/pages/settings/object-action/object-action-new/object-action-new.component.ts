...
@Component({
  selector: 'app-object-action-new',
  standalone: true,
  imports: [
    CommonModule,
    SharedModule,
  ...
  templateUrl: './object-action-new.component.html',
  styleUrl: './object-action-new.component.scss'
})
export class ObjectActionNewComponent implements OnInit {
  ...
  async onSubmit(name: any): Promise<void> {
    ...
    try {
      await this.objectActionService.saveObjectAction({ name: name });
      this.messageService.add({ severity: 'success', summary: 'Sucesso', detail: 'Objeto de Ação cadastrado com sucesso!' });
      setTimeout(() => {
        this.display = false
        location.reload();
      }, 1000);
