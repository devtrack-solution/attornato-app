...
@Component({
  ...
  templateUrl: './free-field1-new.component.html',
  styleUrl: './free-field1-new.component.scss'
})
export class FreeField1NewComponent implements OnInit {
  ...
  async onSubmit(name: any): Promise<void> {
    ...
    try {
      await this.freeField1Service.saveFreeField1({ name: name });
      this.messageService.add({ severity: 'success', summary: 'Sucesso', detail: 'Campo Livre 1 scadastrado com sucesso!' });
      setTimeout(() => {
        this.display = false
        location.reload();
      }, 1000);
