...
export class LocatorService extends BasicService {
   ...
   */
  getLocators(limit: number, offset: number, isActive: boolean = true): Observable<any> {
    const params = { isActive, limit, offset };

    return this.httpClient.get(this.apiUrl, this.getRequestOptions(params));
  }

  getSearchLocators(limit: number, offset: number, isActive: boolean = true, search: any): Observable<any> {
