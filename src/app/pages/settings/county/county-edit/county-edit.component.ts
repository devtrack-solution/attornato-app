...
@Component({
  ...
  styleUrl: './county-edit.component.scss'
})
export class CountyEditComponent implements OnInit {
  ...
  async onSubmit(name: any): Promise<void> {
    ...
    try {
      await this.countyService.ediCounty(this.model.id, { name: name });
      this.messageService.add({ severity: 'success', summary: 'Sucesso', detail: 'Comarca atualizado com sucesso!' });
      setTimeout(() => {
        this.display = false
        location.reload();
      }, 1000);
    } catch (error) {
      console.log('Erro ao salvar Comarca', error);
      this.messageService.add({ severity: 'error', summary: 'Erro', detail: 'Erro ao salvar um Comarca!' });
    }
  }
