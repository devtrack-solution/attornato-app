...
@Component({
  ...
  styleUrl: './county-new.component.scss'
})
export class CountyNewComponent implements OnInit {
  ...
  async onSubmit(name: any): Promise<void> {
    ...
    try {
      await this.countyService.saveCounty({ name: name });
      this.messageService.add({ severity: 'success', summary: 'Sucesso', detail: 'Comarca cadastrado com sucesso!' });
      setTimeout(() => {
        this.display = false
        location.reload();
      }, 1000);
