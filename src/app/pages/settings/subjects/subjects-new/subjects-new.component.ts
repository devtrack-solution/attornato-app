...
@Component({
  ...
  styleUrl: './subjects-new.component.scss'
})
export class SubjectsNewComponent implements OnInit {
  ...
  async onSubmit(name: any): Promise<void> {
    ...
    try {
      await this.subjectService.saveSubject({ name: name });
      this.messageService.add({ severity: 'success', summary: 'Sucesso', detail: 'Assunto cadastrado com sucesso!' });
      setTimeout(() => {
        this.display = false
        location.reload();
      }, 1000);
