...
@Component({
  ...
  styleUrl: './subjects-edit.component.scss'
})
export class SubjectsEditComponent implements OnInit {
  ...
  async openLg(subject: any): Promise<void> {
    this.model = subject
    this.form.patchValue({ name: subject.name }); // Preenche o formulário com os dados do processo
    this.display = true
  }
  ...
  async onSubmit(name: any): Promise<void> {
    ...
    try {
      await this.subjectService.ediSubject(this.model.id, { name: name });
      ...
      this.messageService.add({ severity: 'error', summary: 'Erro', detail: 'Erro ao salvar um Assunto!' });
    }
  }
